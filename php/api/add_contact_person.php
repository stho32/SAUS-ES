...
/**
 * API endpoint to add a contact person to a ticket
...
try {
    // Get contact person details for the comment
    $contactPerson = getContactPerson($contactPersonId);
    if (!$contactPerson) {
        throw new Exception('Ansprechpartner nicht gefunden');
    }
    
    // Link the contact person to the ticket
    $result = linkContactPersonToTicket($ticketId, $contactPersonId);
    
    if ($result) {
        // Add a comment to the ticket history
        $username = getCurrentUsername();
        $formattedInfo = formatContactPersonInfoForComment($contactPerson);
        $commentContent = "Ansprechpartner {$formattedInfo} wurde hinzugefügt.";
        
        $commentSuccess = addTicketComment($ticketId, $commentContent, $username);
        
        if (!$commentSuccess) {
            // If we can't add the comment, but the link was created, it's still a partial success
            echo json_encode([
                'success' => true,
                'message' => 'Ansprechpartner hinzugefügt, aber der Kommentar konnte nicht erstellt werden.'
            ]);
            exit;
        }
        ...
        echo json_encode([
            'success' => true,
            'message' => 'Ansprechpartner erfolgreich hinzugefügt'
        ]);
    } else {
        echo json_encode([
            'success' => false,
            'message' => 'Fehler beim Hinzufügen des Ansprechpartners'
        ]);
    }
} catch (Exception $e) {
